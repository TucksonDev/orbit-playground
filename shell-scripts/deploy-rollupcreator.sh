...
if [[ -z "${CUSTOM_RPC_URL}" || -z "${CUSTOM_PRIVKEY}" || -z "${FACTORY_OWNER}" || -z "${MAX_DATA_SIZE}" || -z "${DISABLE_VERIFICATION}" ]]; then
    echo "Please make sure the following environment variables are set in the .env file:"
    echo "CUSTOM_RPC_URL, CUSTOM_PRIVKEY, FACTORY_OWNER, MAX_DATA_SIZE, DISABLE_VERIFICATION"
    exit 1
fi
...
dotenv -- yarn --cwd nitro-contracts deploy-factory --network custom
